...
BIG THINGS:
! Add a semantic YAML steward tool... the AI feeds it piece by piece, asks it whats missing, and fills it in until its complete ...& make stuart bot and demo 
...
Store thread_id as metadata in slack messages so we can recover on restarts
...
Try more native apps, see best practices:
    Slingshot-"open in worksheet" button at top, then pops a SQL out that creates a share 
        See the SAHRE EVENTS WITH PROVIDER that it does
