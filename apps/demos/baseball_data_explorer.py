...
1. Fetching and displaying infromation on avaialable baseball teams from a demo database, without explicitly specifying what table or query to use.
...
def print_teams_info(client):
    ...
    msg = (
        ...
        "ordered by the total number of games played. The table should contain the following columns: 'team_code', 'team name'. 'total_games_played'. "
...
def get_team_win_lose_ratio(client, team_code, year=None):
    ...
    msg += ("If you have the infromation, return ONLY a text table containing the following information: total wins, total losses, win/lose ratio, and [year, if provided]. "
            "Also provide a proper header for the table. "
            "Do not add any additional text or comments. "
            "If you don't have the information, explain why you don't have the information and sugget a way to get the information.")
    request = client.submit_message(bot_id, msg)
    response = client.get_response(bot_id, request.request_id)
    return response
...
def main():
    ...
    with GenesisAPI(server_proxy=server_proxy) as client:
        ...
        while True:
            ...
            team_code = input(">> To show win/lose ratio for a specific team, enter a team code (or 'quit' to exit): ").strip()
