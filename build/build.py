...
def create_build_directory(version):
    ...
    # # Copy apps directory - DEPRECATED
...
def build_package(build_dir, args):
    ...
    try:
        ...
        if args.sdist:
            ...
            # Move the tar.gz files to build/dist directory
            dist_dir = os.path.join('dist')
            if os.path.exists(dist_dir):
                for file in os.listdir(dist_dir):
                    if file.endswith('.tar.gz'):
                        dest_path = os.path.join('..', file)
                        print(f"\nMoving source distribution to: {dest_path}")
                        shutil.move(
...
def main():
    ...
    parser.add_argument('--sdist', action='store_true', help='Build source distribution instead of wheel')
    ...
    if args.sdist:
        ...
        print(f"Building wheel (Cython compilation {'enabled' if args.cython else 'disabled'})")
    
    # Set COMPILE_CYTHON based on args instead of environment variable
    global COMPILE_CYTHON
    COMPILE_CYTHON = args.cython
    ...
    if args.sdist:
        print("Source distribution has been moved to build/dist directory")
    else:
