...
def _jira_connector(action: str,
    ...
    if action == "GET_ISSUE":
        success = False
        if issue_key:
            result = get_jira_issue(issue_name=issue_key)
            if result:
                success = True
        else:
    ...
    if action == "UPDATE_ISSUE":
        ...
        if issue_key:
            if status:
                status_result = set_issue_status(issue_name=issue_key, status_text=status)
                if status_result:
                    success = True
            ...
            if description:
                comment_result = set_issue_comment(issue_name=issue_key, comment=description)
                if comment_result:
                    success = True
            ...
            if user_name:
                assign_user_result = set_issue_assigned_user(issue_name=issue_key, user_name=user_name)
...
def set_issue_comment(issue_name, comment, thread_id=None):
    try:
        jira_connector = jira_api_connector()
        jira = jira_connector.connect()
...
def set_issue_status(issue_name, status_text, thread_id=None):
    try:
        jira_connector = jira_api_connector()
        jira = jira_connector.connect()
        if jira == True:
...
def get_jira_issue(issue_name, thread_id=None):
    try:
        jira_connector = jira_api_connector()
        jira = jira_connector.connect()
