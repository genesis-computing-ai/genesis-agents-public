...
if llm_api_key is not None:
    (
        ...
        bot_id_to_udf_adapter_map,
        SystemVariables.bot_id_to_slack_adapter_map,
    ) = create_sessions(default_llm_engine, llm_api_key)
else:
    ...
    try:
        ...
        if llm_api_key_candidate is not None:
            ...
            (
                ...
                bot_id_to_udf_adapter_map,
                SystemVariables.bot_id_to_slack_adapter_map,
            ) = create_sessions(llm_api_key, default_llm_engine)
            server = BotOsServer(
                app,
                sessions=sessions,
                scheduler=scheduler,
                scheduler_seoconds_interval=2,
...
if llm_api_key is not None:
    BotOsServer.stream_mode = True
    server = BotOsServer(
        app, sessions=sessions, scheduler=scheduler, scheduler_seoconds_interval=1
    )
    set_remove_pointers(server, api_app_id_to_session_map)
