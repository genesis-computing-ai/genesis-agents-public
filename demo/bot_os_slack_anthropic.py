...
from core.bot_os_defaults import ELSA_DATA_ANALYST_INSTRUCTIONS, ELSA_DATA_ANALYST_INSTRUCTIONS_MISTRAL
...
session = BotOsSession("Elsa_Anthropic", instructions=ELSA_DATA_ANALYST_INSTRUCTIONS_MISTRAL, 
                       ...
                       available_functions={"run_query": bind_run_query([bigquery_connector]),
                                             "search_metadata": bind_search_metadata(BotOsKnowledgeAnnoy_Metadata("./kb_vector")),},
                       tools=database_tool_functions,
                       update_existing=True)

app = Flask(__name__)

scheduler = BackgroundScheduler({'apscheduler.job_defaults.max_instances': 1, 'apscheduler.job_defaults.coalesce': True}) # wish we could move this inside BotOsServer
