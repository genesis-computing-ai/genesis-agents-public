...
from utils import get_metadata, get_bot_details
import re
import uuid
            ...
            with st.container():
                ...
                if isinstance(work_description, str) and 'tmp/' in work_description:
                    ...
                    for match in matches:
                        ...
                        if st.button(f"💻 View Log", key=f"view_file_{file_path}_{entry.get('action_timestamp')}"):
                ...
                if thread_id and thread_id != 'N/A':
                    if st.button(f"🧵 View Thread", key=f"view_thread_{thread_id}"):
                        st.session_state["selected_page_id"] = "file_viewer"
                        st.session_state["radio"] = "File Viewer"
