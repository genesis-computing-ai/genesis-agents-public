...
run = client.beta.threads.runs.create(
    thread_id=knowledge_thread_id, assistant_id=assistant.id
)

while not client.beta.threads.runs.retrieve(
    thread_id=knowledge_thread_id, run_id=run.id
).completed_at:
    time.sleep(1)

response = json.loads(
    client.beta.threads.messages.list(knowledge_thread_id).data[0].content[0].text.value
)
...
prompts = {
    "thread_summary": "Extract a short summary of the conversation",
...
for item, prompt in prompts.items():
    ...
    res = (
        client.beta.threads.messages.list(knowledge_thread_id)
        .data[0]
        .content[0]
        .text.value
    )
