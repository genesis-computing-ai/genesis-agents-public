           ...
           # print(run.status)
            ...
            if run.status == "in_progress":
               ...
               try:
                  ...
                  if run_duration > 60 and run_duration % 60 < 2 and run.id not in StreamingEventHandler.run_id_to_output_stream:  # Check if run duration is beyond 60 seconds and within the first 5 seconds of each subsequent minute
                     event_callback(self.assistant.id, BotOsOutputMessage(thread_id=thread_id, 
                                                                           status=run.status, 
