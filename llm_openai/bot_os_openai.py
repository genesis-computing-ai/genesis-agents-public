            ...
            if self.function_name == "example_blog_post_function":
                ...
                with self.client.beta.threads.runs.submit_tool_outputs_stream(
                    ...
                    event_handler=StreamingEventHandler(
                        self.client, self.thread_id, self.assistant_id
                    ),
                ) as stream:
                    stream.until_done()
            else:
    ...
    def on_event(self, event: AssistantStreamEvent) -> None:
        ...
        try:
            if event.event == "thread.run.created":
                ...
                StreamingEventHandler.run_id_to_bot_assist[self.run_id] = (
                    self.bot_assist
                )
                self.bot_assist.thread_run_map[self.thread_id] = {
                    "run": self.run_id,
                    "completed_at": None,
                }
...
class BotOsAssistantOpenAI(BotOsAssistantInterface):
    ...
    def update_vector_store(
        ...
        try:
            if len(file_streams) > 0:
                ...
                logger.info(
                    f"File counts added to the vector store '{vector_store_id}': local: {file_batch.file_counts}"
                )
            if len(stage_streams) > 0:
                ...
                logger.info(
                    f"File counts added to the vector store '{vector_store_id}': local: {stage_batch.file_counts}"
                )

        except Exception as e:
            logger.error(
                f"Failed to add files to the vector store '{vector_store_id}' for the bot with error: {e}"
            )
            return vector_store_id
        ...
        for file_stream in stage_streams:
            ...
            logger.info(
                f"Vector store '{vector_store_id}' creation status: {stage_batch.status}"
            )
            return vector_store_id
    ...
    def _upload_files(self, files, thread_id=None):
        ...
        for f in files:
            ...
            with open(original_file_location, "rb") as source_file:
                with open(
                    f"./downloaded_files/{thread_id}/{file_name}", "wb"
                ) as dest_file:
        ...
        logger.debug(
            f"BotOsAssistantOpenAI:_upload_files - uploaded {len(file_ids)} files"
        )
        return file_ids, file_map

    def add_message_to_llm(
        ...
        try:
            ...
            file_ids, file_map = self._upload_files(
                input_message.files, thread_id=thread_id
            )
            ...
            for file_id in file_ids:
                ...
                if file_name and any(
                    file_name.lower().endswith(ext) for ext in self.allowed_types_search
                ):
                    tools.insert(0, {"type": "file_search"})
                if file_name and any(
                    file_name.lower().endswith(ext) for ext in self.allowed_types_code_i
                ):
                    tools.append({"type": "code_interpreter"})
                attachments.append({"file_id": file_id, "tools": tools})
            if (
                input_message.metadata
                and input_message.metadata.get("response_authorized", "TRUE") == "FALSE"
            ):
                input_message.msg = (
                    "THIS IS AN INFORMATIONAL MESSAGE ONLY ABOUT ACTIVITY IN THIS THREAD BETWEEN OTHER USERS.  RESPOND ONLY WITH '!NO_RESPONSE_REQUIRED'\nHere is the rest of the message so you know whats going on: \n\n"
                    + input_message.msg
            ...
            try:
                if (
                    "while a run" in e.body.get("message")
                    and self.first_message == True
                ):
                    run_id_match = re.search(
                        r"run_([a-zA-Z0-9]+)", e.body.get("message")
                    )
                    if run_id_match:
        ...
        primary_user = json.dumps(
            {
                "user_id": input_message.metadata.get("user_id", "Unknown User ID"),
        ...
        self.log_db_connector.insert_chat_history_row(
            ...
            files=attachments,
            channel_type=input_message.metadata.get("channel_type", None),
    ...
    def _submit_tool_outputs(
        self, run_id, thread_id, tool_call_id, function_call_details, func_response
    ):

        logger.debug(
            f"_submit_tool_outputs - {thread_id} {run_id} {tool_call_id} - {function_call_details} - {func_response}"
        )
        ...
        if isinstance(func_response, str):
            try:
                ...
                func_response = new_response
                print(
                    f"openai submit_tool_outputs string response converted call: {function_call_details}, response: {func_response}"
                )
            except:
                print(
        ...
        try:
            ...
            if function_call_details[0][0] == "add_new_tools_to_bot" and (
                ...
                if target_bot is not None:
                    ...
                    for assistant in my_assistants:
                        ...
                        if (
                            "snowflake_stage_tools" in all_tools_for_bot
                            and "make_baby_bot" in all_tools_for_bot
                        ):
                            new_instructions += f"\nYour Internal Files Stage for bots is at snowflake stage: {self.genbot_internal_project_and_schema}.BOT_FILES_STAGE"
                            print(
                                "Instruction for target bot updated with Internal Files Stage location."
                            )
                        bot_tools_array = (
                            bot_tools_array
                        ...
                        if "database_tools" in all_tools_for_bot:
                            workspace_schema_name = f"{target_bot}_WORKSPACE".replace(
                                "-", "_"
                            ).upper()
            ...
            if function_call_details[0][0] == "update_bot_instructions" and (
                func_response.get("success", False) == True
                or func_response.get("Success", False) == True
            ):
                ...
                if new_instructions:
                    ...
                    if target_bot is not None:
                        ...
                        if (
                            bot_details["slack_active"] == "Y"
                            and global_flags.slack_active
                        ):
                            instructions += (
                                "\nYour slack user_id: "
                                + bot_details["bot_slack_user_id"]
                            )

                        if (
                            "snowflake_stage_tools" in bot_details["available_tools"]
                            and "make_baby_bot" in bot_details["available_tools"]
                        ):
                        ...
                        if "database_tools" in bot_details["available_tools"]:
                            workspace_schema_name = f"{global_flags.project_id}.{target_bot}_WORKSPACE".replace(
                                "-", "_"
                            ).upper()
                            instructions += f"\nYou have a workspace schema created specifically for you named {workspace_schema_name} that the user can also access. You may use this schema for creating tables, views, and stages that are required when generating answers to data analysis questions. Only use this schema if asked to create an object. Always return the full location of the object."
                        ...
                        for assistant in my_assistants:
                            self.client.beta.assistants.update(
                                assistant.id, instructions=instructions
                            )

                        print(
                            f"Bot instructions for {target_bot} updated: {instructions}"
                        )
    ...
    def _generate_callback_closure(self, run, thread, tool_call_id, function_details):
        def callback_closure(
            func_response,
        ):  # FixMe: need to break out as a generate closure so tool_call_id isn't copied
            try:
            ...
            try:
                self._submit_tool_outputs(
                    run.id, thread.id, tool_call_id, function_details, func_response
                )
            except Exception as e:
                error_string = (
                    f"callback_closure - _submit_tool_outputs - caught exception: {e}"
                )
                print(error_string)
                try:
                    self._submit_tool_outputs(
                        run.id, thread.id, tool_call_id, function_details, error_string
                    )
                except Exception as e:
    ...
    def _download_openai_file(self, file_id, thread_id):
        ...
        try:
            ...
            local_file_path = os.path.join(
                f"./downloaded_files/{thread_id}/", os.path.basename(file_info.filename)
            )
            ...
            try:
                ...
                print(
                    f"{self.bot_name} open_ai download_file - error - couldnt write to {file_id_based_path} err: {e}",
                    flush=True,
                )
                pass
        except Exception as e:
    ...
    def validate_or_add_function(self, function_name):
        ...
        if function_name in self.all_functions:
            ...
            try:
                ...
                logger.warning(
                    f"Function '{function_name}' is not in all_functions. Please add it before proceeding."
                )

            print(
                f"Likely newly generated function '{function_name}' added all_functions."
            )
            return False
        ...
        while thread_id is not None:
            ...
            if thread_run["completed_at"] is None:
                ...
                run = self.client.beta.threads.runs.retrieve(
                    thread_id=thread_id, run_id=thread_run["run"]
                )
                # print(run.status)
                if (
                    (run.status == "in_progress" or run.status == "requires_action")
                    and BotOsAssistantOpenAI.stream_mode == True
                    and run.id in StreamingEventHandler.run_id_to_output_stream
                ):
                ...
                print(
                    f"{self.bot_name} open_ai check_runs ",
                    run.status,
                    " thread: ",
                    thread_id,
                    flush=True,
                ...
                run_duration = (
                    current_time - datetime.datetime.fromtimestamp(run.created_at)
                ).total_seconds()
                if run.status == "in_progress":
                    ...
                    try:
                        ...
                        if (
                            run_duration > 60 and run_duration % 60 < 2
                        ):  # Check if run duration is beyond 60 seconds and within the first 5 seconds of each subsequent minute
                            event_callback(
                                self.assistant.id,
                                BotOsOutputMessage(
                                    thread_id=thread_id,
                ...
                if run.status == "failed":
                    print(
                        f"!!!!!!!!!! FAILED JOB, run.lasterror {run.last_error} !!!!!!!"
                    )
                    ...
                    output = (
                        f"!!! Error from OpenAI, run.lasterror {run.last_error} !!!"
                    )
                    event_callback(
                        self.assistant.id,
                        BotOsOutputMessage(
                ...
                if run.status == "expired":
                    logger.error(
                        f"!!!!!!!!!! EXPIRED JOB, run.lasterror {run.last_error} !!!!!!!"
                    )
                ...
                if run.status == "requires_action":
                    ...
                    try:
                        if not all(
                            key in self.tool_completion_status[run.id]
                            for key in parallel_tool_call_ids
                        ):
                            self.tool_completion_status[run.id] = {
                                key: None for key in parallel_tool_call_ids
                            }
                    except:
                        self.tool_completion_status[run.id] = {
                            key: None for key in parallel_tool_call_ids
                        }  # need to submit completed parallel calls together
                    thread = self.client.beta.threads.retrieve(thread_id)
                    try:
                        for func_name, func_args, tool_call_id in function_details:
                            if (
                                tool_call_id in self.running_tools
                            ):  # already running in a parallel thread
                                continue
                            if (
                                self.tool_completion_status[run.id].get(
                                    tool_call_id, None
                                )
                                is not None
                            ):
                            ...
                            if (
                                False
                                and self.tool_completion_status[run.id].get(
                                    tool_call_id, None
                                )
                                is not None
                            ):
                                try:
                                    if tool_call_id in self.callback_closures:
                                        ...
                                        result = self.tool_completion_status[run.id][
                                            tool_call_id
                                        ]
                                        print(
                                            "Tool is done but results not submitted. Calling back again."
                                        )
                                        try:
                                            callback(result)
                                            print(
                                    ...
                                    print(
                                        "Tool completed but callback failed. Running tool again."
                                    )
                                    pass
                            log_readable_payload = func_name + "(" + func_args + ")"
                            try:
                                callback_closure = self._generate_callback_closure(
                                    run, thread, tool_call_id, function_details
                                )
                                self.callback_closures[tool_call_id] = callback_closure
                            except Exception as e:
                                print(
                                    f"Failed to generate callback closure for run {run.id}, thread {thread.id}, tool_call_id {tool_call_id} with error: {e}"
                                )
                            self.running_tools[tool_call_id] = {
                                "run_id": run.id,
                                "thread_id": thread.id,
                            }
                            ...
                            self.log_db_connector.insert_chat_history_row(
                                ...
                                message_payload=log_readable_payload,
                                message_metadata={
                            ...
                            if (
                                "image_data" in func_args_dict
                            ):  # FixMe: find a better way to convert file_id back to stored file
                                func_args_dict["image_data"] = self.file_storage.get(
                                    func_args_dict["image_data"].removeprefix(
                                        "/mnt/data/"
                                    )
                                )
                            ...
                            if "file_name" in func_args_dict:
                                ...
                                try:
                                    if func_args_dict["file_name"].startswith(
                                        "/mnt/data/"
                                    ):
                                        file_id = func_args_dict["file_name"].split(
                                            "/"
                                        )[-1]
                                        new_file_location = self.file_storage[file_id]
                                        if "/" in new_file_location:
                                            new_file_location = new_file_location.split(
                                                "/"
                                            )[-1]
                                    ...
                                    logger.warn(
                                        f"Failed to update file_name in func_args_dict with error: {e}"
                                    )

                            if "openai_file_id" in func_args_dict:

                                try:
                                    file_id = func_args_dict["openai_file_id"].split(
                                        "/"
                                    )[-1]
                                    existing_location = (
                                        f"./downloaded_files/{thread_id}/{file_id}"
                                    )
                    ...
                    and run.completed_at != thread_run["completed_at"]
                ):
                ...
                logger.debug(
                    f"check_runs - {thread_id} - {run.status} - {run.completed_at} - {thread_run['completed_at']}"
                )

                # record completed runs.  FixMe: maybe we should rmeove from the map at some point?
                for thread_id in threads_completed:
                    self.thread_run_map[thread_id]["completed_at"] = threads_completed[
                        thread_id
                    ]
