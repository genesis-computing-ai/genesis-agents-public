...
class BotOsAssistantReka(BotOsAssistantInterface):
   def __init__(self, name:str, instructions:str, 
                tools:list[dict], available_functions={}, files=[], update_existing=False,
                log_db_connector=None, bot_id='default_bot_id', bot_name='default_bot_name') -> None:
        logger.debug("BotOsAssistantReka:__init__") 
        api_key = os.environ["REKA_API_KEY"]
        reka.API_KEY = api_key
        self.model_name = os.getenv("REKA_MODEL_NAME", default="fantastic-lily-20240327")
...
'''
   ...
   def create_thread(self) -> str:
        ...
        self.thread_messages_map[thread] =  [ {"type": "human", "text": self.instructions } , { "type": "model", "text": "I'm ready."}]
        #response = reka.chat(
        #self.instructions ,
        #conversation_history=[],
   ...
   def _upload_files(self, files):
      ...
      logger.debug(f"BotOsAssistantReka:_upload_files - uploaded {len(file_ids)} files") 
      return file_ids
   ...
   def add_message(self, input_message:BotOsInputMessage):#thread_id:str, message:str, files):
      ...
      self.thread_messages_map[thread_id].append({"type": "human", "text": input_message.msg})

      print(response["text"])
      print(response["metadata"])

      self.thread_messages_map[thread_id].append({"type": "model", "text": response["text"]})
   ...
   def check_runs(self, event_callback):
      ...
      if function_call is not None:
         ...
         new_event = BotOsInputMessage(thread_id=recent_thread["thread_id"], msg=str(result), 
                     metadata=recent_thread["metadata"])
         self.add_message(new_event)
      else:
         o = BotOsOutputMessage( recent_thread["thread_id"],"completed",recent_thread["response"],[], 
