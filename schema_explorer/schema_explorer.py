...
class SchemaExplorer:
    def __init__(self, db_connector):
        ...
        self.cortex_embedding_model = os.getenv(
            "CORTEX_EMBEDDING_MODEL", "nv-embed-qa-4"
        )
    ...
    def alt_get_ddl(self, table_name=None):
        ...
        try:
            describe_result = self.db_connector.run_query(
                query=describe_query, max_rows=1000, max_rows_override=True
            )
        except:
            return None
        ...
        for column in describe_result:
            ...
            if column.get("primary_key", False):
                ...
                key = " UNIQUE"
            ddl_statement += (
                f"    {column_name} {column_type}{nullable}{default}{key}{comment},\n"
            )
    ...
    def store_table_memory(
        ...
        try:
            ...
            self.store_table_summary(
                ...
                schema,
                table,
                summary="Harvester Error: {e}",
                ddl="Harvester Error",
                ddl_short="Harvester Error",
                sample_data="Harvester Error",
            )

    def store_table_summary(
        self, database, schema, table, ddl, ddl_short="", summary="", sample_data=""
    ):
        """
        ...
        try:
            ...
            self.db_connector.insert_table_summary(
                schema_name=schema,
    ...
    def generate_summary(self, prompt):
        p = [
            {
                "role": "system",
                "content": "You are an assistant that is great at explaining database tables and columns in natural language.",
            },
    ...
    def run_prompt(self, messages):
        if self.db_connector.cortex_mode:
            ...
            completion_result = self.db_connector.run_query(
                f"select snowflake.cortex.complete('{self.cortex_model}','{escaped_messages}')"
            )
            try:
    ...
    def get_ddl_short(self, ddl):
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an assistant that is great at taking full table DDL and creating shorter DDL summaries.",
            },
            {"role": "user", "content": prompt},
        ]
    ...
    def get_embedding(self, text):
        if False and self.db_connector.cortex_mode:
            ...
            embedding_result = self.db_connector.run_query(
                f"SELECT SNOWFLAKE.CORTEX.EMBED_TEXT_1024('{self.cortex_embedding_model}', '{escaped_messages}');"
            )
            try:
    ...
    def generate_column_summary_prompt(
        self, database, schema, table, column, sample_values
    ):
    ...
    def get_datasets(self, database):
        try:
            datasets = self.db_connector.get_schemas(
                database
            )  # Assume this method exists and returns a list of dataset IDs
            return datasets
        except Exception as e:
    ...
    def explore_and_summarize_tables_parallel(self, max_to_process=1000):
        try:
            ...
            for database in databases:
                ...
                if crawl_flag:
                    ...
                    print(
                        f'Checking a Database for new or changed objects (cycle#: {self.run_number}, refresh every: {database["refresh_interval"]}) {cur_time}',
                        flush=True,
                    )
                else:
                    print(
                        f'Skipping a Database, not in current refresh cycle (cycle#: {self.run_number}, refresh every: {database["refresh_interval"]} {cur_time})',
                        flush=True,
                    )
        ...
        def process_dataset_step1(dataset, max_to_process=1000):
            ...
            if self.db_connector.source_name == "Snowflake":
                try:
                    potential_tables = self.db_connector.get_tables(
                ...
                table_names = [
                    table_info["table_name"] for table_info in potential_tables
                ]
                ...
                """
                try:
                    existing_tables_info = self.db_connector.run_query(
                        check_query, max_rows=1000, max_rows_override=True
                    )
                    existing_tables_set = {
                        info["QUALIFIED_TABLE_NAME"] for info in existing_tables_info
                    }
                    non_existing_tables = [
                        table
                        for table in potential_tables
                        if f'"{db}"."{sch}"."{table["table_name"]}"'
                        not in existing_tables_set
                    ]
                    needs_updating = [
                        table["QUALIFIED_TABLE_NAME"]
                        for table in existing_tables_info
                        if table["NEEDS_FULL"]
                    ]
                    refresh_tables = [
                        table
                        for table in potential_tables
                        if f'"{db}"."{sch}"."{table["table_name"]}"' in needs_updating
                    ]
                except Exception as e:
                ...
                for table_info in non_existing_tables:
                    try:
                        ...
                        if (
                            quoted_table_name not in existing_tables_set
                            or quoted_table_name in needs_updating
                        ):
                            ...
                            if sch == "INFORMATION_SCHEMA":
                                shared_table_exists = (
                                    self.db_connector.check_cached_metadata(
                                        "PLACEHOLDER_DB_NAME", sch, table_name
                                    )
                                )
                            else:
                                shared_table_exists = (
                            ...
                            if shared_table_exists:
                                ...
                                get_from_cache_result = (
                                    self.db_connector.get_metadata_from_cache(
                                        db, sch, table_name
                                    )
                                )
                                ...
                                current_ddl_hash = (
                                    self.db_connector.sha256_hash_hex_string(
                                        current_ddl
                                    )
                                )
                                new_table = {
                                ...
                                print(
                                    "Newly found object added to harvest array (no cache hit)",
                                    flush=True,
                                )
                ...
                """
                try:
                    non_indexed_tables = self.db_connector.run_query(
                        query, max_rows=max_to_process, max_rows_override=True
                    )
                except Exception as e:
        ...
        def process_dataset_step2(non_indexed_tables, max_to_process=1000):
            ...
            if len(non_indexed_tables) > 0:
                print(
                    f"starting indexing of {len(non_indexed_tables)} objects...",
                    flush=True,
                )
            for row in non_indexed_tables:
                try:
                    ...
                    self.store_table_memory(
                        database, schema, table, summary, ddl=ddl, ddl_short=ddl_short
                    )
                except Exception as e:
        ...
        try:
            print(
                f"Checking {len(schemas)} schemas for new (not changed) objects.",
                flush=True,
            )
        except Exception as e:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_dataset = {
                executor.submit(process_dataset, schema, max_to_process): schema
                for schema in schemas
            ...
            for future in as_completed(future_to_dataset):
                ...
                try:
                    ...
                    print(
                        f"Dataset {dataset} generated an exception: {exc}", flush=True
                    )
