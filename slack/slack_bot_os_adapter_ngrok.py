# I DONT THINK THIS FILE IS NEEED ...-JL
...
class SlackBotAdapter(BotOsInputAdapter):
    def __init__(self, token:str, signing_secret:str, channel_id:str, bot_user_id:str, bot_name:str='Unknown') -> None:
...
#        return self.handler.handle(request)
    ...
    def _download_slack_files(self, event) -> list:
        files = []
    ...
    def get_input(self) -> BotOsInputMessage|None:
       ...
       # print("get_input bot ", self.bot_user_id, 'len events: ', len(self.events))
        ...
        if 'files' in event:
            files = self._download_slack_files(event)
        thread_ts = event.get('thread_ts', event.get('ts', ''))
