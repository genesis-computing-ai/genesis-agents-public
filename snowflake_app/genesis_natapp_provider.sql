...
-- ########## SCRIPTS CONTENT  ###########################################
USE SCHEMA GENESISAPP_APP_PKG.CODE_SCHEMA;
CREATE TABLE IF NOT EXISTS GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT (NAME VARCHAR, VALUE VARCHAR);
DELETE FROM SCRIPT;




INSERT INTO GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT (NAME , VALUE)
VALUES ('MANIFEST',
$$
...
-- delete from script;
INSERT INTO GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT SELECT * FROM GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT_TMP;


CREATE OR REPLACE TEMPORARY TABLE GENESISAPP_APP_PKG.CODE_SCHEMA.script_tmp AS SELECT 'SETUP' NAME,REGEXP_REPLACE($$
...
CREATE OR REPLACE PROCEDURE APP.CREATE_KNOWLEDGE_SERVICE(INSTANCE_NAME VARCHAR,SERVICE_NAME VARCHAR, POOL_NAME VARCHAR, EAI_NAME VARCHAR , WAREHOUSE_NAME VARCHAR, APP_DATABASE VARCHAR)
RETURNS VARCHAR NOT NULL
LANGUAGE SQL
AS
:::
...
BEGIN
  ...
  CALL APP.CREATE_KNOWLEDGE_SERVICE(:INSTANCE_NAME,'GENESISAPP_KNOWLEDGE_SERVICE',:POOL_NAME, :EAI_NAME, :APP_WAREHOUSE, :v_current_database);
...
CALL APP.UPGRADE_APP('APP1','GENESISAPP_KNOWLEDGE_SERVICE', FALSE, CURRENT_DATABASE(),'GENESIS_EAI','GENESIS_POOL');
...
CALL GENESISAPP_APP_PKG..CODE_SCHEMA.PUT_TO_STAGE('APP_CODE_STAGE','manifest.yml',(SELECT VALUE FROM GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT WHERE NAME = 'MANIFEST'));
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.GET_FROM_STAGE('APP_CODE_STAGE','manifest.yml');
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.PUT_TO_STAGE('APP_CODE_STAGE','setup_script.sql', (SELECT VALUE FROM GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT WHERE NAME = 'SETUP'));
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.GET_FROM_STAGE('APP_CODE_STAGE','setup_script.sql');
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.PUT_TO_STAGE('APP_CODE_STAGE','readme.md', (SELECT VALUE FROM GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT WHERE NAME = 'README'));
CALL GENESISAPP_APP_PKG.GENESISAPP_APP_PKG.CODE_SCHEMA.GET_FROM_STAGE('APP_CODE_STAGE','readme.md');
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.PUT_TO_STAGE_SUBDIR('APP_CODE_STAGE','code_artifacts/streamlit','sis_launch.py', (SELECT VALUE FROM GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT WHERE NAME = 'SIS_APP'));
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.GET_FROM_STAGE_SUBDIR('APP_CODE_STAGE','code_artifacts/streamlit','sis_launch.py');
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.PUT_TO_STAGE_SUBDIR('APP_CODE_STAGE','code_artifacts/streamlit','environment.yml', (SELECT VALUE FROM GENESISAPP_APP_PKG.CODE_SCHEMA.SCRIPT WHERE NAME = 'SIS_ENV'));
CALL GENESISAPP_APP_PKG.CODE_SCHEMA.GET_FROM_STAGE_SUBDIR('APP_CODE_STAGE','code_artifacts/streamlit','environment.yml');
...
BEGIN
LET rs0 RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_APP_PKG ADD VERSION V0_3 USING @GENESISAPP_APP_PKG.CODE_SCHEMA.APP_CODE_STAGE');
RETURN TABLE(rs0);
EXCEPTION
 WHEN OTHER THEN
   LET rs1 RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_APP_PKG ADD PATCH FOR VERSION V0_3 USING @GENESISAPP_APP_PKG.CODE_SCHEMA.APP_CODE_STAGE');
   RETURN TABLE(rs1);
END;
;
...
-- CREATE APPLICATION GENESIS_BOTS_ALPHA FROM APPLICATION PACKAGE GENESISAPP_APP_PKG USING VERSION V0_3;
...
BEGIN
 ...
 select max("patch") INTO :max_patch FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) where "version" = 'V0_3';
 LET rs RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_APP_PKG SET DEFAULT RELEASE DIRECTIVE VERSION = V0_3 PATCH = '||:max_patch);
 RETURN TABLE(rs);
END;
