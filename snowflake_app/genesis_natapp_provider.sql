...
GRANT REFERENCE_USAGE ON DATABASE SPIDER_DATA TO SHARE IN APPLICATION PACKAGE GENESISAPP_APP_PKG;
GRANT REFERENCE_USAGE ON DATABASE GENESISAPP_MASTER TO SHARE IN APPLICATION PACKAGE GENESISAPP_APP_PKG;
...
CREATE OR REPLACE PROCEDURE CODE_SCHEMA.SHARE_HARVEST_RESULTS(APP_NAME STRING, APP_PKG_NAME STRING)
RETURNS STRING
LANGUAGE SQL
AS
$$
...
-- Share harvest records from provider to app package
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || APP_PKG_NAME || '.SHARED_HARVEST.HARVEST_RESULTS AS 
...
CALL CODE_SCHEMA.SHARE_HARVEST_RESULTS('GENESIS_BOTS','GENESISAPP_APP_PKG');
...
CREATE OR REPLACE PROCEDURE APP.UPGRADE_APP(INSTANCE_NAME VARCHAR, SERVICE_NAME VARCHAR, UPDATE_HARVEST_METADATA BOOLEAN)
...
BEGIN
    ...
    IF (:schema_exists) then
      ...
      IF (UPDATE_HARVEST_METADATA) THEN
        ...
        SELECT COUNT(*) > 0 INTO :harvest_schema_exists FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'APP1' AND TABLE_NAME = 'HARVEST_RESULTS';

        WITH CHECK_DB_EXISTS AS
        (SELECT COUNT(*) DB FROM APP1.HARVEST_CONTROL WHERE DATABASE_NAME = 'GENESIS_BOTS' 
        ), CHECK_EXCLUSION_LIST AS
        (SELECT COUNT(*) EXCLUSION FROM APP1.HARVEST_CONTROL
          WHERE DATABASE_NAME = 'GENESIS_BOTS' AND (ARRAY_CONTAINS('BASEBALL'::variant, SCHEMA_EXCLUSIONS) OR ARRAY_CONTAINS('FORMULA_1'::variant, SCHEMA_EXCLUSIONS))
        )
        SELECT IFF(DB=0,1, IFF(EXCLUSION>0,1,0)) INTO :harvest_excluded
        FROM CHECK_DB_EXISTS, CHECK_EXCLUSION_LIST;

        IF  (:harvest_schema_exists AND NOT :harvest_excluded) THEN
          EXECUTE IMMEDIATE 'DELETE FROM APP1.HARVEST_RESULTS WHERE SCHEMA_NAME IN (''BASEBALL'', ''FORMULA_1'')';
          EXECUTE IMMEDIATE 'INSERT INTO APP1.HARVEST_RESULTS (SOURCE_NAME, QUALIFIED_TABLE_NAME, DATABASE_NAME, MEMORY_UUID, SCHEMA_NAME, TABLE_NAME, COMPLETE_DESCRIPTION, DDL, DDL_SHORT, DDL_HASH, SUMMARY, SAMPLE_DATA_TEXT, LAST_CRAWLED_TIMESTAMP, CRAWL_STATUS, ROLE_USED_FOR_CRAWL, EMBEDDING)
                            SELECT SOURCE_NAME, QUALIFIED_TABLE_NAME, DATABASE_NAME, MEMORY_UUID, SCHEMA_NAME, TABLE_NAME, COMPLETE_DESCRIPTION, DDL, DDL_SHORT, DDL_HASH, SUMMARY, SAMPLE_DATA_TEXT, LAST_CRAWLED_TIMESTAMP, CRAWL_STATUS, ROLE_USED_FOR_CRAWL, EMBEDDING FROM SHARED_HARVEST.HARVEST_RESULTS';
        END IF;      
      END IF;
    END IF;
...
CALL APP.UPGRADE_APP('APP1','GENESISAPP_SERVICE_SERVICE', TRUE);
-- upgrades harvester services, but will not update harvest results again
CALL APP.UPGRADE_APP('APP1','GENESISAPP_HARVESTER_SERVICE', FALSE);
...
BEGIN
LET rs0 RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_APP_PKG ADD VERSION V0_1 USING @GENESISAPP_APP_PKG.CODE_SCHEMA.APP_CODE_STAGE');
RETURN TABLE(rs0);
EXCEPTION
 WHEN OTHER THEN
   LET rs1 RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_APP_PKG ADD PATCH FOR VERSION V0_1 USING @GENESISAPP_APP_PKG.CODE_SCHEMA.APP_CODE_STAGE');
   RETURN TABLE(rs1);
END;
;
...
SET APP_DATABASE='GENESIS_BOTS_ALPHA';
CREATE APPLICATION GENESIS_BOTS_ALPHA FROM APPLICATION PACKAGE GENESISAPP_APP_PKG USING VERSION V0_1;
