...
--DROP DATABASE IF EXISTS GENESISAPP_DEV_APP_PKG ;
--drop database genesisapp_dev_master;
...
CREATE APPLICATION PACKAGE IF NOT EXISTS GENESISAPP_DEV_APP_PKG;


USE DATABASE GENESISAPP_DEV_APP_PKG;
CREATE SCHEMA IF NOT EXISTS CODE_SCHEMA;
...
--
USE SCHEMA GENESISAPP_DEV_MASTER.CODE_SCHEMA;
show image repositories;

-- ########## UTILITY FUNCTIONS  #########################################
USE SCHEMA GENESISAPP_DEV_APP_PKG.CODE_SCHEMA;
use warehouse xsmall;
...
-- Grant reference usage on additional databases
  ...
  EXECUTE IMMEDIATE 'GRANT REFERENCE_USAGE ON DATABASE GENESISAPP_DEV_MASTER TO SHARE IN APPLICATION PACKAGE ' || APP_PKG_NAME || ';';
...
-- Share harvest records from provider to app package (to be removed)
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || APP_PKG_NAME || '.SHARED_HARVEST.HARVEST_RESULTS AS SELECT * FROM GENESISAPP_DEV_MASTER.HARVEST_SHARE.HARVEST_RESULTS_SHARED';
 ...
 -- Share harvest data
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || APP_PKG_NAME || '.APP_SHARE.HARVEST_RESULTS AS SELECT * FROM GENESISAPP_DEV_MASTER.HARVEST_SHARE.HARVEST_RESULTS_SHARED';
 ...
 -- Share image data
  EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW ' || APP_PKG_NAME || '.APP_SHARE.IMAGES AS SELECT ID, IMAGE_NAME, BOT_NAME, ENCODED_IMAGE_DATA, IMAGE_DESC FROM GENESISAPP_DEV_MASTER.APP_SHARE.IMAGES_SHARED';
...
artifacts:
 ...
 container_services:
   images:
   - /genesisapp_dev_master/code_schema/service_repo/genesis_app:latest  
 extension_code: true
...
`COMPUTE POOL`GENESIS_DEV_POOL
For Genesis to use to run its Genesis Server containers

`DATABASE`GENESIS_DEV_LOCAL_DB
To store the network rule

`SCHEMA`GENESIS_DEV_LOCAL_DB.SETTINGS
To store the network rule

`NETWORK RULE`GENESIS_DEV_LOCAL_DB.SETTINGS.GENESIS_DEV_RULE
To allow Genesis to access to required external APIs (OpenAI and Slack)

`EXTERNAL ACCESS INTEGRATION`GENESIS_DEV_EAI
To allow Genesis to access to required external APIs (OpenAI and Slack)
...
set APP_DATABASE = 'GENESIS_DEV_BOTS_ALPHA';
...
GRANT USAGE ON COMPUTE POOL GENESIS_DEV_POOL TO APPLICATION  IDENTIFIER($APP_DATABASE);
...
:::
    spec:
      containers:
      - name: genesis
        image: /genesisapp_dev_master/code_schema/service_repo/genesis_app:latest
        env:
...
BEGIN
    ...
    -- REVOKE USAGE ON FUNCTION APP1.get_metadata(varchar) FROM APPLICATION ROLE APP_PUBLIC;
      ...
      THEN
         ...
         EXECUTE IMMEDIATE 
           'GRANT SERVICE ROLE '|| :INSTANCE_NAME ||'.'|| :SERVICE_NAME || '!ALL_ENDPOINTS_USAGE TO APPLICATION ROLE APP_PUBLIC';
        END IF;
...
CALL APP.UPGRADE_APP('APP1','GENESISAPP_SERVICE_SERVICE', TRUE, CURRENT_DATABASE(),'GENESIS_DEV_EAI','GENESIS_DEV_POOL');
-- upgrades harvester services, but will not update harvest results again
CALL APP.UPGRADE_APP('APP1','GENESISAPP_HARVESTER_SERVICE', FALSE, CURRENT_DATABASE(),'GENESIS_DEV_EAI','GENESIS_DEV_POOL');
-- upgrades or creates if needed task automation services, but will not update harvest results again
CALL APP.UPGRADE_APP('APP1','GENESISAPP_TASK_SERVICE', FALSE, CURRENT_DATABASE(),'GENESIS_DEV_EAI','GENESIS_DEV_POOL');
...
BEGIN
LET rs0 RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_DEV_APP_PKG ADD VERSION V0_2 USING @GENESISAPP_DEV_APP_PKG.CODE_SCHEMA.APP_CODE_STAGE');
RETURN TABLE(rs0);
EXCEPTION
 WHEN OTHER THEN
   LET rs1 RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_DEV_APP_PKG ADD PATCH FOR VERSION V0_2 USING @GENESISAPP_DEV_APP_PKG.CODE_SCHEMA.APP_CODE_STAGE');
   RETURN TABLE(rs1);
END;
;
...
// DROP APPLICATION IF EXISTS GENESIS_DEV_BOTS_ALPHA;
// show applications;

// SET APP_DATABASE='GENESIS_DEV_BOTS_ALPHA';

// CREATE APPLICATION GENESIS_DEV_BOTS_ALPHA FROM APPLICATION PACKAGE GENESISAPP_DEV_APP_PKG USING VERSION V0_2;


-- ########## BEGIN PUBLISH   ############################################


ALTER APPLICATION PACKAGE GENESISAPP_DEV_APP_PKG
...
DECLARE
 max_patch VARCHAR;
BEGIN
 show versions in application package GENESISAPP_DEV_APP_PKG;
 select max("patch") INTO :max_patch FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) where "version" = 'V0_2';
 LET rs RESULTSET := (EXECUTE IMMEDIATE 'ALTER APPLICATION PACKAGE GENESISAPP_DEV_APP_PKG SET DEFAULT RELEASE DIRECTIVE VERSION = V0_2 PATCH = '||:max_patch);
 RETURN TABLE(rs);
END;
