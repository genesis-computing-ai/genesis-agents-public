...
CREATE PROCEDURE IF NOT EXISTS CORE.REGISTER_SINGLE_REFERENCE(ref_name STRING, operation STRING, ref_or_alias STRING)
  RETURNS STRING
  LANGUAGE SQL
  AS $$
    BEGIN
      CASE (operation)
        WHEN 'ADD' THEN
          ...
          IF (ref_name = 'CONSUMER_WAREHOUSE') THEN
            //create services
            CALL CORE.INITIALIZE_APP_INSTANCE('APP1','GENESIS_POOL',NULL,NULL);
          END IF;
        WHEN 'REMOVE' THEN
      ...
      END CASE;
      RETURN ref_name || ' registered. ' || ref_or_alias;
    END;
  $$;

GRANT USAGE ON PROCEDURE CORE.REGISTER_SINGLE_REFERENCE(STRING, STRING, STRING)
