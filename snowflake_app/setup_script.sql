    ...
    spec:
      ...
      - name: genesis
        ...
        env:
            ...
            OPENAI_MODEL_SUPERVISOR: o1-preview
    ...
    spec:
      ...
      - name: genesis-harvester
        ...
        env:
            ...
            OPENAI_HARVESTER_EMBEDDING_MODEL: text-embedding-3-large
            OPENAI_MODEL_NAME: gpt-4o-2024-08-06
            OPENAI_MODEL_SUPERVISOR: o1-mini
...
CREATE OR REPLACE PROCEDURE core.grant_callback(privileges array)
 ...
 BEGIN
   ...
   IF (ARRAY_CONTAINS('CREATE WAREHOUSE'::VARIANT, privileges)) THEN
      ...
      CALL CORE.INITIALIZE_APP_INSTANCE('APP1','GENESIS_POOL',NULL,'APP_XSMALL');
...
BEGIN
    ...
    REVOKE USAGE ON FUNCTION APP1.get_metadata(varchar) FROM APPLICATION ROLE APP_PUBLIC;
      ...
      IF (EAI_NAME IS NOT NULL AND EAI_NAME != '') THEN
        EXECUTE IMMEDIATE
          'ALTER SERVICE IF EXISTS '|| :INSTANCE_NAME ||'.'|| :SERVICE_NAME ||
          ' SET ' ||
      ...
      -- if (WH_NAME is not NULL)
      -- THEN
      --   IF (EAI_NAME IS NOT NULL AND EAI_NAME != '')
      --   THEN
      --     EXECUTE IMMEDIATE
...
BEGIN
  ...
  CALL APP.CREATE_SERVER_SERVICE(:INSTANCE_NAME,'GENESISAPP_SERVICE_SERVICE',:POOL_NAME, :EAI_NAME, :WAREHOUSE_NAME, :v_current_database);
  CALL APP.CREATE_HARVESTER_SERVICE(:INSTANCE_NAME,'GENESISAPP_HARVESTER_SERVICE',:POOL_NAME, :EAI_NAME, :WAREHOUSE_NAME, :v_current_database);
  CALL APP.CREATE_KNOWLEDGE_SERVICE(:INSTANCE_NAME,'GENESISAPP_KNOWLEDGE_SERVICE',:POOL_NAME, :EAI_NAME, :WAREHOUSE_NAME, :v_current_database);
  CALL APP.CREATE_TASK_SERVICE(:INSTANCE_NAME,'GENESISAPP_TASK_SERVICE',:POOL_NAME, :EAI_NAME, :WAREHOUSE_NAME, :v_current_database);
...
BEGIN
    ...
    WHERE TABLE_NAME = 'CUSTOM_CONFIG_SETTING' 
    AND TABLE_SCHEMA = 'APP1';

    IF (TABLE_EXISTS) THEN
        SELECT CUSTOM_OBJECT_NAME INTO EAI_NAME 
        FROM APP1.CUSTOM_CONFIG_SETTING 
        WHERE CUSTOM_OBJECT_TYPE = 'EAI';
    ELSE
        EAI_NAME := NULL;
    END IF;

    -- Upgrade services
    CALL APP.UPGRADE_APP('APP1', 'GENESISAPP_SERVICE_SERVICE', TRUE, CURRENT_DATABASE(), :EAI_NAME, 'GENESIS_POOL', :WAREHOUSE_NAME);
    CALL APP.UPGRADE_APP('APP1', 'GENESISAPP_HARVESTER_SERVICE', FALSE, CURRENT_DATABASE(), :EAI_NAME, 'GENESIS_POOL', :WAREHOUSE_NAME);
    CALL APP.UPGRADE_APP('APP1', 'GENESISAPP_KNOWLEDGE_SERVICE', FALSE, CURRENT_DATABASE(), :EAI_NAME, 'GENESIS_POOL', :WAREHOUSE_NAME);
    CALL APP.UPGRADE_APP('APP1', 'GENESISAPP_TASK_SERVICE', FALSE, CURRENT_DATABASE(), :EAI_NAME, 'GENESIS_POOL', :WAREHOUSE_NAME);
