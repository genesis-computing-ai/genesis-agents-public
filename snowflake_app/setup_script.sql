  ...
  BEGIN
   ...
   IF (ARRAY_CONTAINS('CREATE WAREHOUSE'::VARIANT, privileges)) THEN
     ...
     BEGIN
      ...
      SELECT "name" INTO wh_exists FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) WHERE "name" = 'APP_XSMALL' AND "owner" = current_database() LIMIT 1;
     EXCEPTION
      WHEN STATEMENT_ERROR THEN
       wh_exists := '';
     END;

      IF (wh_exists IS NOT NULL AND wh_exists <> '') THEN
        wh_name := 'APP_XSMALL';
      ELSE
