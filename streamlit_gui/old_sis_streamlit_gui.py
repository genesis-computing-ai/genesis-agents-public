...
def llm_config():  # Check if data is not empty
    ...
    if True:
        ...
        st.write(
            "Genesis Bots require access to OpenAI or Reka LLMs, as these are the only models currently powerful enough to service the bots. Please visit https://platform.openai.com/signup or https://platform.reka.ai/ to get a paid API key for OpenAI or Reka before proceeding."
        )
...
def setup_slack():
    ...
    st.write(
        "By providing a Slack App Config Token and Refresh Token, Genesis Bots can create, update, and remove Genesis Bots from your Slack environment.  Go to https://api.slack.com/apps and create an App Config Token and App Config Refresh Token, paste them below, and press Update. "
    )
        ...
        for index, row in available_databases_df.iterrows():
            ...
            if st.button(f"Add {database_name} to Harvest", key=f"add_{database_name}"):
                ...
                if add_to_harvest_response.get("Success"):
                    st.success(
                        f"Database {database_name} added to harvest successfully."
                    )
                else:
                    st.error(
                        f"Failed to add {database_name} to harvest: {add_to_harvest_response.get('Error')}"
                    )
    else:
                ...
                for bot in bot_details:
                    ...
                    with col1:
                        ...
                        if (
                            ...
                            st.markdown(
                                f"**Authorize on Slack:** [Link]({bot['auth_url']})"
                            )
                            ...
                            if slack_ready and (
                                ...
                                if st.button(
                                    ...
                                    if deploy_response.get("Success"):
                                        ...
                                        #   st.write(bot)
                                        st.success(
                                            f"Bot {bot.get('bot_name')} deployed to Slack successfully. [Click here to Authorize on Slack]({deploy_response.get('auth_url')})"
                                        )
                                    else:
                                        st.error(
