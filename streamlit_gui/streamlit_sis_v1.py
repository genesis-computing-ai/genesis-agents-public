...
def llm_config():  # Check if data is not empty
    ...
    if True:
        ...
        # Display the active LLM type in bold

        if llm_model in ["OpenAI", "gemini"]:
            selected_key = [llm["llm_key"] for llm in llm_info if llm["llm_type"] == llm_model]
            if selected_key:
                cur_key = selected_key[0][:3] + "*" * (len(selected_key[0]) - 7) + selected_key[0][-4:] if len(selected_key[0]) > 10 else selected_key[0]  # Show first 6 and last 4 characters
