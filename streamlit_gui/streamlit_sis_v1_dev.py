            ...
            if wh_test:
                try:
                    with st.spinner('Starting Compute Pool & Genesis Server (can take 3-15 minutes the first time for compute pool startup, use "show compute pools;" to see status)...'):
                        start_result = session.sql(f"call {core_prefix}.INITIALIZE_APP_INSTANCE('APP1','GENESIS_DEV_POOL','GENESIS_DEV_EAI','{st.session_state.wh_name}')")
                        ...
                        if st.button('Click to start Genesis Service'):
                            with st.spinner('Genesis Services is starting...'):
                                try:
                                    # Execute the command and collect the results
                                    time.sleep(15)
                                    service_start_result = session.sql(f"call {app_name}.core.start_app_instance('APP1','GENESIS_DEV_POOL','GENESIS_DEV_EAI','{st.session_state.wh_name}')").collect()
