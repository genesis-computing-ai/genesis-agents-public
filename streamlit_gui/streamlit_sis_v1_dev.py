...
def submit_to_udf_proxy(input_text, thread_id, bot_id):
    ...
    if NativeMode:
        ...
        data = session.sql(
            sql, (input_text, thread_id, json.dumps(primary_user))
        ).collect()
        response = data[0][0]
        return response
...
def show_server_logs():
    ...
    log_type = st.selectbox(
        "Select log type:", ["Bot Service", "Harvester", "Task Service"]
    )

    if log_type == "Bot Service":
        # Run Snowflake SQL commands for Bot Service
        status_result = session.sql(
            f"SELECT SYSTEM$GET_SERVICE_STATUS('{prefix}.GENESISAPP_SERVICE_SERVICE')"
        ).collect()
        logs_result = session.sql(
            f"SELECT SYSTEM$GET_SERVICE_LOGS('{prefix}.GENESISAPP_SERVICE_SERVICE',0,'genesis',1000)"
        ).collect()
        ...
        status_result = session.sql(
            f"SELECT SYSTEM$GET_SERVICE_STATUS('{prefix}.GENESISAPP_HARVESTER_SERVICE')"
        ).collect()
        logs_result = session.sql(
            f"SELECT SYSTEM$GET_SERVICE_LOGS('{prefix}.GENESISAPP_HARVESTER_SERVICE',0,'genesis-harvester',1000)"
        ).collect()
        ...
        # Run Snowflake SQL commands for Harvester
        status_result = session.sql(
            f"SELECT SYSTEM$GET_SERVICE_STATUS('{prefix}.GENESISAPP_TASK_SERVICE')"
        ).collect()
        logs_result = session.sql(
            f"SELECT SYSTEM$GET_SERVICE_LOGS('{prefix}.GENESISAPP_TASK_SERVICE',0,'genesis-task-server',1000)"
        ).collect()
...
def support():
    ...
    st.markdown(
        "## [Join our Slack Community](https://communityinviter.com/apps/genesisbotscommunity/genesis-bots-community)"
    )
...
def llm_config():  # Check if data is not empty
    ...
    if True:
        ...
        if st.button(
            "Submit API Key", key="sendllm", disabled=st.session_state.disable_submit
        ):
...
def setup_ngrok():
    ...
    if ngrok_active:
        ...
        st.warning(
            "Ngrok is not currently active, please complete the form below to activate."
        )

    st.title("Ngrok Tokens Setup")
    st.write(
        "By providing a Ngrok Auth Token, the Slack Connnector will be able to talk to your Genesis Bots. Go to https://dashboard.ngrok.com/signup and create an free Ngrok account, and paste the Ngrok Auth Token below."
    )
    # Text input for Slack App Token
    ...
    if st.button("Update Ngrok Tokens"):
        ...
        if resp.get("Success") == False or resp.get("ngrok_active") == False:
            st.error(
                f"Invalid Ngrok Token or other Activation Error: {resp.get('Message','')}"
            )
        else:
...
def db_add_to_harvester():
    ...
    try:
        ...
        if available_databases:
            ...
            st.dataframe(
                available_databases_df[["DatabaseName", "Schemas"]],
            ...
            if available_databases_df.shape[0] > 0:
                ...
                if "INFORMATION_SCHEMA" in schemas:
                    ...
                    example_text = (
                        f"Eve, please add {first_database} and include all schemas"
                    )
            st.write(
                f"To add a database to the harvest, just ask Eve and provide the database name and schemas to include or exclude."
            )
            st.write(f'For example, "{example_text}"')

        else:
            st.write(
                "No other data currently visible but not already being harvested. By default Genesis auto-harvests visible data, so see Harvester Status to see if granted data may already be in the harvest process. See the Grant Data Access tab on left for instructions to grant additional access to data to this application."
            )
    except:
        st.write(
            "No other data currently visible but not already being harvested. By default Genesis auto-harvests visible data, so see Harvester Status to see if granted data may already be in the harvest process. See the Grant Data Access tab on left for instructions to grant additional access to data to this application."
...
USE SCHEMA GENESIS_DEV_LOCAL_DB.SETTINGS;
USE WAREHOUSE XSMALL; -- or use your warehouse if not XSMALL
"""
        + """

CREATE OR REPLACE PROCEDURE GENESIS_DEV_LOCAL_DB.SETTINGS.grant_schema_usage_and_select_to_app(database_name STRING, APP_NAME STRING)
...
call GENESIS_DEV_LOCAL_DB.SETTINGS.grant_schema_usage_and_select_to_app('<your db name>',$APP_DATABASE);
                    ...
                    with col1:
                        ...
                        if user_id == "Pending_APP_LEVEL_TOKEN":
                            ...
                            if st.button(
                                ...
                                provide_slack_level_key_response = (
                                    provide_slack_level_key(
                                        bot["bot_id"], slack_app_level_key
                                    )
                                )
                                # st.write(provide_slack_level_key_response)
                                if provide_slack_level_key_response.get(
                                    "success", False
                                ):
                                    st.success(
                                        "Slack App Level Key provided successfully."
                                    )
                                    st.markdown(
                                        f"**To complete setup on Slack, there is one more step.  Cut and paste this link in a new browser window (appologies that it can't be clickable here):**"
                                    )
                        ...
                        if (
                            ...
                            st.markdown(
                                f"**To complete setup on Slack, there is one more step.  Cut and paste this link in a new browser window (appologies that it can't be clickable here).  You may need to log into both Slack and Snowflake to complete this process:**"
                            )
                            # st.text(f"{bot['auth_url']}")
                            st.text_area(
                                "Link to use to authorize:",
                        ...
                        if bot["slack_active"] == "N" or bot["slack_deployed"] == False:
                            if slack_ready and (
                                ...
                                if st.button(
                                    ...
                                    if deploy_response.get(
                                        "Success"
                                    ) or deploy_response.get("success"):
                                        # st.write(deploy_response)
                                        # st.write(bot)
                                        st.success(
                        ...
                        if slack_user_allow is not None:
                            ...
                            if allowed_users is not None:
                                if "!BLOCK_ALL" in allowed_users:
                                    st.caption(
                                        f"Allowed Slack Users: None - All Blocked"
                                    )
                                else:
                                    st.caption(
                                        f"Allowed Slack Users IDs: {allowed_users} (Eve can tell you who these are)"
                                    )
                            else:
...
def config_pool():
    ...
    st.write(
        "Genesis Bots has a server component that runs securely inside your Snowflake account, that coordinates the actions of your Genesis Bots, and manages their interactions with other users and bots. To run this server, you need to create and grant Genesis Server access to a Snowflake Compute Pool."
    )
    st.write(
...
DROP COMPUTE POOL IF EXISTS GENESIS_DEV_POOL;

-- create the compute pool and associate it to this application
CREATE COMPUTE POOL IF NOT EXISTS GENESIS_DEV_POOL FOR APPLICATION IDENTIFIER($APP_DATABASE)
...
GRANT USAGE, OPERATE ON COMPUTE POOL GENESIS_DEV_POOL TO APPLICATION  IDENTIFIER($APP_DATABASE);
"""

    st.text_area("Compute Pool configuration script:", wh_text, height=420)

    st.write(
        "We can't automatically test this, but if you've performed it same way you did on Step 1, you can now proceed to the next step."
    )
...
CREATE DATABASE IF NOT EXISTS GENESIS_DEV_LOCAL_DB; 
CREATE SCHEMA IF NOT EXISTS GENESIS_DEV_LOCAL_DB.SETTINGS;
...
CREATE OR REPLACE NETWORK RULE GENESIS_DEV_LOCAL_DB.SETTINGS.GENESIS_DEV_RULE
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION GENESIS_DEV_EAI
   ALLOWED_NETWORK_RULES = (GENESIS_DEV_LOCAL_DB.SETTINGS.GENESIS_DEV_RULE) ENABLED = true;
...
GRANT USAGE ON INTEGRATION GENESIS_DEV_EAI TO APPLICATION   IDENTIFIER($APP_DATABASE);

-- (optional steps for event logging) 
-- create a schema to hold the event table
CREATE SCHEMA IF NOT EXISTS GENESIS_DEV_LOCAL_DB.EVENTS;

-- create an event table to capture events from the Genesis Server
CREATE EVENT TABLE  IF NOT EXISTS GENESIS_DEV_LOCAL_DB.EVENTS.GENESIS_DEV_APP_EVENTS;
...
ALTER ACCOUNT SET EVENT_TABLE=GENESIS_DEV_LOCAL_DB.EVENTS.GENESIS_DEV_APP_EVENTS;
...
"""
    ...
    st.write(
        "Once you run the above, you can proceed to the next step to start the Genesis Server."
    )
                ...
                try:
                    with st.spinner(
                        ...
                        start_result = session.sql(
                            f"call {core_prefix}.INITIALIZE_APP_INSTANCE('APP1','GENESIS_DEV_POOL','GENESIS_DEV_EAI','{st.session_state.wh_name}')"
                        )
...
alter compute pool GENESIS_DEV_POOL SUSPEND; -- to also pause the compute pool

// resume service

alter compute pool GENESIS_DEV_POOL RESUME; -- if you paused the compute pool
call {app_name}.core.start_app_instance('APP1','GENESIS_DEV_POOL','GENESIS_DEV_EAI','{st.session_state.wh_name}'); 
...
CALL {app_name}.CORE.INITIALIZE_APP_INSTANCE('APP1','GENESIS_DEV_POOL','GENESIS_DEV_EAI','{st.session_state.wh_name}');
                            ...
                            with st.spinner("Genesis Services is starting..."):
                                try:
                                    ...
                                    service_start_result = session.sql(
                                        f"call {app_name}.core.start_app_instance('APP1','GENESIS_DEV_POOL','GENESIS_DEV_EAI','{st.session_state.wh_name}')"
                                    ).collect()
                                    if service_start_result:
                                        service_status.text(
                                            "Genesis Service status: "
